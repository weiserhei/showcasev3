	...
	*/
	...
	Enemy.prototype = {
		...
		onClick: function() {
			...
			this._standardMaterial = this.mesh.material;

			var highlitedMaterial = this.mesh.material.clone();
			if( highlitedMaterial instanceof THREE.MultiMaterial ) {     
				var innermaterial = highlitedMaterial.materials[0]; 
				innermaterial.emissive.setHex( 0x0000FF );
			}
			this.mesh.material = highlitedMaterial;

		},

		onDeselect: function() {
			this._ring.visible = false;
			this.mesh.material = this._standardMaterial;
		},
		...
		_setupFSM: function() {
			...
			var fsm = StateMachine.create({
				...
				// States: idle, fighting, dead, running

				initial: 'idle',
				events: [
					{ name: 'wait', from: '*', to: 'idle' },
					{ name: 'run', from: ['fighting','idle','running'], to: 'running' },
					{ name: 'attack', from: ['running','fighting','idle'], to: 'fighting' },
	    ...
	    _getEnemyInRange: function( enemys ) {
            ...
            for ( let i = 0; i < enemys.length; i ++ ) {
                ...
                if ( distance < 1 && ( enemy.fsm.is("running") || enemy.fsm.is("idle") ) ) {
            		return enemy;
                }
            }
	    },
    	...
    	update: function( deltaTime, enemys ) {
            ...
            if ( this.fsm.is("running") || this.fsm.is("idle") ) {
            	var enemy = this._getEnemyInRange( enemys );
            	if( enemy !== undefined ) {
